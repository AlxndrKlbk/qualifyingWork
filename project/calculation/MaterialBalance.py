...
def MB_calculation(Nx, Ny, DesignVariant, calculation_steps):
    ...
    a_matrix_water = np.zeros((Ny * Nx, Nx * Ny), dtype="float32")  # a_matrix{y,x] где у - номер ячейки, для которой считаем производную, x - по которой
    b_matrix_water = np.zeros((Ny * Nx), dtype="float32")   # один b для каждой ячейки
